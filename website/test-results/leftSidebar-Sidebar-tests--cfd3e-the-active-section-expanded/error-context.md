...
- Name: leftSidebar.spec.ts >> Sidebar tests in small screen size (overlay mode) >> navigating between sections keeps the active section expanded
- Location: src/tests/leftSidebar.spec.ts:137:3
...
Locator: locator('aside.left-sidebar .sidebar-section-header').first()
...
Call log:
  ...
  - waiting for locator('aside.left-sidebar .sidebar-section-header').first()
    3 × locator resolved to <div class="sidebar-section-header css-69i1ev">…</div>
      - unexpected value "sidebar-section-header css-69i1ev"
