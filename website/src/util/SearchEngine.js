...
/**
 * Attempts to load a pre-built search index from /search-index.json.
 * This file is generated by `npm run export-index` and placed in the
 * public/ directory at build time, reducing the number of individual
 * markdown fetches needed to initialize the search index.
 *
 * Falls back to building the index from individual markdown files when
 * the pre-built JSON is not available (e.g. during local development
 * before the export script has been run).
 */
async function tryLoadPrebuiltIndex() {
  try {
    const base = import.meta.env?.BASE_URL ?? "/";
    const url = `${base}search-index.json`.replace(/\/\//g, "/");
...
export async function initializeIndex() {
  ...
  // Try to load pre-built index first (faster: single fetch vs many file imports)
  ...
  // Fallback: build index by loading and processing each markdown file
