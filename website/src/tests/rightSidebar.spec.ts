...
test.describe("Right sidebar tests in small screen size (overlay mode)", () => {
  ...
  test("clicking drawer links opens right sidebar and updates URL (overlay mode)", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator(`a.srch-drawer-link[data-term="${termKey}"]`),
  ...
  test('closing via "X" or re-clicking the term closes right sidebar and removes slug (overlay mode)', async ({
    page,
  }) => {
    const { termKey } = await getFirstDrawerChip(page);
    const chip = page
      .locator(`a.srch-drawer-link[data-term="${termKey}"]`)
      .first();
  ...
  test("right sidebar is scrollable or configured to scroll (overlay mode)", async ({
    page,
  }) => {
  ...
  test("opening a second drawer term updates active chip and keeps sidebar open (overlay mode)", async ({
    page,
  }) => {
  ...
  test("in overlay mode, opening the right sidebar does not change main margins", async ({
    page,
  }) => {
    await expect(page.locator("html")).toHaveAttribute(
      "data-layout-mode",
  ...
  test("opening a drawer link preserves main content scroll position (overlay mode)", async ({
    page,
  }) => {
    const main = page.locator(mainContent());
    const { chip } = await getFirstDrawerChip(page);
    ...
    await main.evaluate((el) => {
      el.scrollTo({ top: 600, behavior: "auto" });
    });
...
test.describe("Right sidebar tests in full screen size (wide mode)", () => {
  ...
  test("clicking drawer links opens right sidebar and pushes main content (wide mode)", async ({
    page,
  }) => {
    await expect(page.locator("html")).toHaveAttribute(
      "data-layout-mode",
  ...
  test("opening a drawer link preserves main content scroll position after wide-mode transition", async ({
    page,
  }) => {
    const main = page.locator(mainContent());
    const { chip } = await getFirstDrawerChip(page);
  ...
  test('closing via "X" or re-clicking the term collapses right sidebar and resets main margin (wide mode)', async ({
    page,
  }) => {
    const main = page.locator(mainContent());
    const { termKey } = await getFirstDrawerChip(page);
  ...
  test("right sidebar width can be resized with the drag handle (wide mode)", async ({
    page,
  }) => {
    const { chip } = await getFirstDrawerChip(page);
    await chip.click();
    ...
    if (!initialBox)
      throw new Error("Right sidebar bounding box not available");

    const hitboxBox = await hitbox.boundingBox();
    if (!hitboxBox)
      throw new Error("Right resize hitbox bounding box not available");
    ...
    if (!resizedBox)
      throw new Error("Right sidebar bounding box after resize not available");
  ...
  test("navigating to a different section/subsection auto-closes the right sidebar (wide mode)", async ({
    page,
  }) => {
    const { chip } = await getFirstDrawerChip(page);
  ...
  test("resizing between wide and overlay modes keeps drawer behavior sane and restores wide layout", async ({
    page,
  }) => {
    const main = page.locator(mainContent());
    const { chip } = await getFirstDrawerChip(page);
