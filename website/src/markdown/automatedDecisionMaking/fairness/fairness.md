...
However, there are various challenges in defining and utilizing sensitive attributes. Many fields have legal and privacy constraints that restrict the collection and use of sensitive data, and explicit use of sensitive attributes can be controversial and raise ethical concerns about reinforcing group distinctions. Furthermore, sensitive attributes are inherently ambiguous and complex. Attributes like race or gender are socially constructed, fluid, and intersectional, complicating their operationalization in algorithms.

A persistent issue in this domain is the **{proxy-problem}**. Even when sensitive attributes are excluded from a model, other variables may act as proxies because they correlate strongly with protected characteristics. Example proxies for sensitive attributes include:
...
- {Independence-vs-Separation}: Enforcing demographic parity (independence) may require ignoring legitimate differences in group distributions, potentially reducing utility or harming individual fairness. Example:

- {Separation-vs-Sufficiency}: Equalizing error rates (separation) may conflict with ensuring that prediction scores mean the same thing across groups (sufficiency). Example:

- {Accuracy-vs-Fairness}: Accuracy focuses on prediction performance (e.g., minimizing overall error). Fairness adjustments may require introducing constraints that degrade performance for some groups to satisfy fairness criteria. Example:

- {Individual-vs-Group-Fairness}: Satisfying group fairness often requires treating individuals differently to compensate for historical inequality, which violates individual fairness principles. Enforcing individual fairness can perpetuate group disparities if the similarity metric is derived from biased data. Example:
...
- Regulatory translation: Formal fairness definitions have informed legal and policy discourse, including the Algorithmic Discrimination Protections section of the **AI Bill of Rights**. Statistical parity and disparate impact metrics appear in draft standards and regulatory frameworks across government and compliance frameworks.
...
Where _X_ is the space of feature vectors, _Y_ is the true outcome, _A_ is a sensitive attribute, and _Ŷ_ \= _f (X)_ is the prediction*,* independence holds when _Ŷ_ ⊥ _A._
...
- Does not constrain error rates, allows false positive and negative rates to be unevenly distributed.
...
Where _X_ is the space of feature vectors, _Y_ is the true outcome, _A_ is a sensitive attribute, and _Ŷ_ \= _f (X)_ is the prediction*,* separation holds when _Ŷ_ ⊥ \*A | Y.\*
...
Where _X_ is the space of feature vectors, _Y_ is the true outcome, _A_ is a sensitive attribute, and _Ŷ_ \= _f (X)_ is the prediction*,* sufficiency holds when _Y_ ⊥ \*A | Ŷ\*
...
[“Equality of Opportunity in Supervised Learning”](https://arxiv.org/pdf/1610.02413) by Hardt, Price & Srebro (2016)
...
[“An Ethical Argument for a Group Fairness Definition Insurers Can Use”](https://link.springer.com/article/10.1007/s13347-023-00624-9) by Baumann & Loi (2023)
...
“[Fairness in Criminal Justice Risk Assessments: The State of the Art](https://arxiv.org/abs/1703.09207)” by Berk et al. (2017)
...
Study: [“A Unified Approach to Quantifying Algorithmic Unfairness: Measuring Individual & Group Unfairness via Inequality Indices”](https://doi.org/10.48550/arXiv.1807.00787) by Speicher et al. (2018)
...
In [“The Possibility of Fairness: Revisiting the Impossibility Theorem in Practice,”](https://dl.acm.org/doi/pdf/10.1145/3593013.3594007) Bell et al. (2023) show that if fairness metrics are treated as approximate rather than exact constraints, many models can satisfy multiple fairness criteria simultaneously. Using analytical arguments and experiments, the authors argue that the impossibility theorem is less restrictive in practice, and approximate fairness along multiple metrics is often achievable.

In [“Escaping the Impossibility of Fairness: From Formal to Substantive Algorithmic Fairness,”](https://link.springer.com/article/10.1007/s13347-022-00584-6) Green (2022) argues for a shift from formal fairness metrics to substantive fairness, emphasizing that fairness must be grounded in social context, institutional practices, and normative reasoning.

In [“Pushing the Limits of Fairness Impossibility: Who’s the Fairest of Them All?,”](https://proceedings.neurips.cc/paper_files/paper/2022/file/d3222559698f41247261b7a6c2bbaedc-Paper-Conference.pdf) Hsu et al. (2022) propose a post-processing algorithm that optimizes across multiple fairness criteria simultaneously, mitigating incompatibilities between independence, separation, and sufficiency without requiring perfect balance.

In [“What’s Impossible about Algorithmic Fairness?,”](https://link.springer.com/article/10.1007/s13347-024-00814-z) Sahlgren (2024) provides a philosophical analysis of the impossibility results in algorithmic fairness, examining what kind of “impossibility” they establish and how this relates to broader questions about feasibility and the norms we attach to fairness criteria.
